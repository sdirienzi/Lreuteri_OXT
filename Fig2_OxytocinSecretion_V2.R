...
LifeGiftLreuplot<-ggplot(LGLreusecmerge, aes(x=Segment,y=Oxtnorm,fill=Treatment,group=SegmentTreatment)) +
...
# Data: humandata
# Models:
#   mmtestnull: Oxtnorm ~ (1 | LifeGift)
# mmtest: Oxtnorm ~ Treatment * Segment + (1 | LifeGift)
# npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# mmtestnull    3 2463.4 2473.6 -1228.7   2457.4                         
# mmtest       18 2343.7 2405.0 -1153.9   2307.7 149.68 15  < 2.2e-16 ***
#   ---
...
pig5<-read.delim(file="FigS2A_pig5xdata_forR.txt",header=TRUE,sep="\t")
...
# Data: pigmeltnoKrebs
# Models:
#   mmnull: OXTareafoldscaled ~ (1 | Pig)
# mmtest: OXTareafoldscaled ~ Treatment * Segment + (1 | Pig)
# npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# mmnull    3 609.99 617.49 -302.00   603.99                         
# mmtest   12 509.45 539.45 -242.72   485.45 118.55  9  < 2.2e-16 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
performance::r2(mmtest)
# R2 for Mixed Models
...
piglet<-read.delim(file="FigS2b_Piglet5xdataforR.txt",header=TRUE,sep="\t")
...
# Data: pigletdata
# Models:
#   mmnull: OXTareafoldscaled ~ (1 | Sex)
# mmtest: OXTareafoldscaled ~ Treatment * Intestinal.Segment + (1 | Sex)
# npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# mmnull    3 293.04 297.96 -143.52   287.04                         
# mmtest   10 249.92 266.29 -114.96   229.92 57.128  7  5.638e-10 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
#Fig 2C
#load in mouse
mouse<-read.delim(file="Fig2c_CombinedMouseSecretionData.txt",sep="\t",header=TRUE)
mousearea<-read.delim(file="Fig2_mousearea.txt",sep="\t",header=TRUE)
...
mousesexes<-ggplot(mouse, aes(x=Segment, y=OXTareafoldscaled,fill=Treatment,group=SegmentTreatment, shape=Sex)) +  
  geom_boxplot(size=0.6,outlier.shape = NA,alpha = 0.4,colour="black")+  #, lty=FAMI
  ...
  scale_x_discrete(labels=c("Stomach", "SI","Cecum", "LI"))+
  scale_fill_npg()+
  xlab("Segment") +
  ylab(bquote('Oxytocin (pg/ml/'*cm^2*')')) +
  theme_bw() +
...
pdf("Mouse_2.pdf", width=2.5,height=1.5)
grid.arrange(mousesexes)        
dev.off()
...
#Fig2D
luminex<-read.delim(file="Fig2d_Luminex6475.txt",header=TRUE,sep="\t",check.names=FALSE)
...
# Data: luminexmelt
# Models:
#   HIEnull: Oxytocin ~ (1 | Replicate)
# HIElm: Oxytocin ~ Treatment * Induction + (1 | Replicate)
# npar    AIC BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# HIEnull    3 292.47 296 -143.23   286.47                         
# HIElm      6 255.93 263 -121.97   243.93 42.537  3  3.085e-09 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# Induction = Uninduced:
#   contrast        estimate   SE df t.ratio p.value
# LDM4 - Lreu6475    -48.8 24.7 18  -1.978  0.0586
# 
# Induction = Induced:
#   contrast        estimate   SE df t.ratio p.value
...
#Supp 2E
others<-read.delim(file="Fig2e_others.txt",header=TRUE,sep="\t",check.names=FALSE)
othersmelt<-others
othersmelt$Treatment<-factor(othersmelt$Treatment,c("LDM4","Lreu6475","Bsubtilis","Nissle"))

shapevalues2<-c(21,22,23)
names(shapevalues2)<-levels(as.factor(othersmelt$Replicate))

pdf(file="Others.pdf",width=1.6,height=2)
ggplot(subset(othersmelt, othersmelt$Induction=="Induced"), aes(x=Treatment,y=OxytocinCorrected,fill=Treatment)) +
  geom_boxplot(size=1,outlier.shape = NA,alpha = 0.4,colour="black")+  #, lty=FAMI
...
Figothersmodel<-lmer(OxytocinCorrected~Treatment+ (1|Replicate),data=othersmelt, control=lmerControl(optimizer = "bobyqa"))
Figothersnullmodel<-lmer(OxytocinCorrected~(1|Replicate),data=othersmelt, control=lmerControl(optimizer = "bobyqa"))
anova(Figothersmodel,Figothersnullmodel)
# Data: othersmelt
# Models:
#   Figothersnullmodel: OxytocinCorrected ~ (1 | Replicate)
# Figothersmodel: OxytocinCorrected ~ Treatment + (1 | Replicate)
# npar    AIC    BIC  logLik deviance Chisq Df Pr(>Chisq)    
# Figothersnullmodel    3 274.22 277.63 -134.11   268.22                        
# Figothersmodel        6 212.50 219.32 -100.25   200.50 67.72  3  1.313e-14 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
performance::r2(Figothersmodel)

# Conditional R2: NA
# Marginal R2: 0.940

Figothersemmeans<-emmeans(Figothersmodel,pairwise~Treatment,adjust="BH")
# $contrasts
# contrast             estimate   SE   df t.ratio p.value
# LDM4 - Lreu6475      -192.774 12.7 17.3 -15.147  <.0001
# LDM4 - Bsubtilis        0.904 12.0 17.0   0.075  0.9409
# LDM4 - Nissle           4.221 12.0 17.0   0.351  0.9409
# Lreu6475 - Bsubtilis  193.678 12.7 17.3  15.218  <.0001
# Lreu6475 - Nissle     196.994 12.7 17.3  15.479  <.0001
# Bsubtilis - Nissle      3.317 12.0 17.0   0.276  0.9409
# 
# Degrees-of-freedom method: kenward-roger 
# P value adjustment: BH method for 6 tests 

eff_size(Figothersemmeans,sigma = sigma(Figothersmodel), edf=17)
contrast               effect.size    SE   df lower.CL upper.CL
(LDM4 - Lreu6475)          -9.2643 1.702 17.3   -12.85    -5.68
(LDM4 - Bsubtilis)          0.0434 0.577 17.0    -1.17     1.26
(LDM4 - Nissle)             0.2028 0.578 17.0    -1.02     1.42
(Lreu6475 - Bsubtilis)      9.3078 1.709 17.3     5.71    12.91
(Lreu6475 - Nissle)         9.4672 1.735 17.3     5.81    13.12
(Bsubtilis - Nissle)        0.1594 0.578 17.0    -1.06     1.38

sigma used for effect sizes: 20.81 
Degrees-of-freedom method: inherited from kenward-roger when re-gridding 
Confidence level used: 0.95
